...
Running AI agent frameworks on managed Kubernetes isn't just about convenience. OpenShift provides network policies, role-based access control (RBAC), and image security scanning that matter when your orchestration layer is dispatching tasks to LLM providers. Agent orchestration systems handle API keys, model endpoints, and potentially sensitive code context. Deploying them on a platform with built-in security controls reduces the blast radius of misconfigurations.

Red Hat OpenShift AI adds GPU scheduling and model serving capabilities, which means Omnigent can eventually dispatch to locally-hosted models alongside cloud APIs. For this proof of concept, we focused on getting the core orchestration running with external LLM providers.
...
The `imagePullSecrets` entry is easy to forget. Our first deployment attempt sat in `ImagePullBackOff` for several minutes before we realized the namespace was missing the Quay registry credentials. On OpenShift, you can create the secret and link it to the default service account:
...
Omnigent is open source at [github.com/databricks/omnigent](https://github.com/databricks/omnigent). If you've got access to an OpenShift cluster, you can reproduce this deployment using AutoPoC, which automates the entire pipeline from repository analysis to validation testing.

To get started with Red Hat OpenShift AI, visit [developers.redhat.com/products/red-hat-openshift-ai](https://developers.redhat.com/products/red-hat-openshift-ai). The free developer sandbox gives you a cluster to experiment with, no credit card required.

The Dockerfile and Kubernetes manifests from this PoC are available in the [autopoc-artifacts branch](https://github.com/databricks/omnigent/tree/autopoc-artifacts) of the forked repository.
